...
describe('Emitter', () => {
	...
	describe('on', () => {
		...
		it('should skip initial data', () => {
			const cb = jest.fn();
			const data = { data: 'important' };

			const emitter = new Emitter();
			emitter._get = data;
			emitter.on('a', cb, true);
	...
	describe('onDeep', () => {
		it('should not fail on invalid parameters', () => {
			const emitter = new Emitter();
			expect(emitter.onDeep()).toBe(emitter);
			emitter.onDeep('');
			emitter.onDeep('a');
			emitter.onDeep('b', 0);
			emitter.onDeep('c', '', () => 0);
			emitter.onDeep('d', -1, () => 0);
			expect(emitter.getRelatedListeners('')).toEqual(['d']);
		});

		it('should create a deep listener', () => {
			const data = {
				'a.b$keys': ['c', 'd'],
				'a.b.c': 1,
				'a.b.d': 2,
			};
			const emit = key => emitter.emit(key, emitter.get(key));

			const emitter = new Emitter();
			emitter.get = (name) => data[name];

			const cb = jest.fn();
			emitter.onDeep('a.b', 1, cb);
			emit('a.b$keys');

			return common.delay(1).then(() => {
				expect(cb).toBeCalledWith('c', 1);
				expect(cb).toBeCalledWith('d', 2);
			});
		});

		it('should create a really deep listener', () => {
			const data = {
				'a$keys': ['b'],
				'a.b$keys': ['c', 'd'],
				'a.b.c': 1,
			...
			const cb = jest.fn();
			emitter.onDeep('a', 2, cb);
			emit('a$keys');

			return common.delay(1).then(() => {
				expect(cb).toBeCalledWith('b.c', 1);
				expect(cb).toBeCalledWith('b.d', 2);
			});
		});

		it('should create a deep listener on root', () => {
			const data = {
				'$keys': ['a'],
				'a$keys': ['b'],
				'a.b': 1,
			...
			return common.delay(1).then(() => {
				expect(cb).toBeCalledWith('a.b', 1);
			});
		});

		it('should clean up its listeners', () => {
			const data = {
				'a$keys': ['b'],
				'a.b$keys': ['c', 'd'],
				'a.b.c': 1,
