...
class Socketio {
	...
	getSocketRooms(socket) {
		...
		return Object.keys(socket.rooms)
			.filter(k => k.startsWith(preface))
			.map(k => k.slice(preface.length));
	}

	getRoomSockets(io, key) {
		return Object.entries(io.sockets)
			.filter(([id, socket]) => this.getSocketRooms(socket).includes(key))
			.map(([id]) => id);
	}

	makeRoom(io, key) {
		console.log('makeRoom', key);
		if (!this.rooms[key]) {
			this.rooms[key] = data => {
				io.to(`${this.ioRoomKey}.${key}`).emit(`${this.opts.uri}${key}`, data);
			};
			this.db.on(key, this.rooms[key]);
		}
		return `${this.ioRoomKey}.${key}`;
	}

	delRoom(io, key) {
		console.log('delRoom', key);
		if (this.rooms[key]) {
	...
	cleanRooms(io) {
		console.log('cleanRooms');
		return Object.keys(this.rooms).forEach(key => {
			if (this.getRoomSockets(io, key).length < 1) this.delRoom(io, key);
		});
	}

	joinRoom(socket, key) {
		console.log('joinRoom', key);
		const id = this.makeRoom(socket.server, key);
	...
	createMidware() {
		console.log('createMidware');
		this.midware = (socket, next) => {
