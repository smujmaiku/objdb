...
describe('common', () => {
	...
	describe('eachKey', () => {
		it('should callback with keys', () => {
			...
			const cb = jest.fn((k, v, m, e) => [k, v, m, e]);

			const res = common.eachKey(obj, cb, ['meta', 'extra']);
	...
	describe('promiseMap', () => {
		it('should return a Promise', () => {
			expect(common.promiseMap()).toEqual(expect.any(Promise));
		});

		it('should resolve promises in order', () => {
			const list = [ () => Promise.resolve(1), () => 2 ];
			return common.promiseMap(list).then((res) => {
